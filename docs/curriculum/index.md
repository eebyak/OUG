...
- **[New Study — B.Sc. Computer Science Programme Document](new-study-programme-document.md)** — the programme-level description of the OUG's first proposed degree. It defines the 180-ECTS stacking model, the six competency streams, module-level microcredentials, the Capstone and Bachelor Thesis, and the relationship between the OUG and the existing DHBW New Study programme. The full skill profiles are automatically generated with this [code](build_skill_profile_atlas.py) resulting in [Skill Profile](OUG_Skill_Profile_Atlas.md) and [Skill Profile (docx)](OUG_Skill_Profile_Atlas.docx). A skills analysis for both AI-skills and human-skills can be found in [the AI/Human Skills Profile](New_Study_AI_Human_Capabilities.md). In addition, we pay attention to both AI and Human skill as shown in our [AI declaration](DDL_AI_and_Human_Roles.md). statements that we share with students.

- **[Modulhandbuch — Bachelor of Computer Science](modulhandbuch.md)** — defines the academic rationale, qualification goals, six competency streams, recommended six-semester progression, open-pathway logic, assessment principles, and canonical structure for module descriptions. The current document establishes the curriculum architecture and includes one worked module example, but does not yet contain a completed catalogue of all individual modules. Machine-assembled Module handbook using json files in skill-based-modules directly uses [python code](build_module_handbook_docx_md.py) and results in [OUG Module Handbook](OUG_Module_Handbook.md) or [OUG Module Handbook (docx)](OUG_Module_Handbook.docx). 
...
The **[New Study Module Catalogue](https://newstudy.campuscircle.de/)** provides a live, searchable view of the curriculum.

Readers can explore modules by name or code and filter them using characteristics such as competency type, domain, skill, Bloom level, module type, language, ECTS, workload, and didactic format.

The catalogue demonstrates the practical difference between a static module handbook and a navigable curriculum system: the same academic information can be approached through a learner's interests, existing competencies, professional needs, or intended pathway.
...
The **Presence Architecture** addresses the social and intellectual conditions required for learning in an asynchronous-first environment.

The **Platform Framework**, the **machine-readable module data**, and the **live catalogue** translate the curriculum from a static document into an explorable learning system.

The **Research and Doctoral Framework** extends the academic architecture beyond the first degree and defines how research, doctoral education, and research-informed teaching operate within the federated model.
